...
define('NETWORKPUB_PLUGIN_ADMIN_URL', 				__("plugins.php?page=networkpub"));
...
define('NETWORKPUB_PLUGIN_VERSION',                 '5.0.1');
...
function networkpub_post_meta_box( $object, $box ) {
	...
	$html .= '&nbsp;<label for="networkpub_meta_cb_publish">'.__('Publish this').' '.ucfirst($this_post_type).' '.__('to').' <a href="plugins.php?page=networkpub">'.__('configured Networks').'</a></label>';
	$html .= '</div>';
	//Content
	...
	if(empty($options['api_key'])) {
		if (!isset($_POST['submit'])) {
			...
			<div class="updated fade" style="padding:10px;text-align:left">
				...
				<div>'.__('You must').' <a href="plugins.php?page=networkpub">'.__('enter API key').'</a> '.__(' on the settings page for plugin').' '.NETWORKPUB_WIDGET_NAME.' '.__('for automatic posting of your blog articles to 25+ Social Networks including Twitter, Facebook Profile, Facebook Pages, LinkedIn, MySpace, Yammer, Yahoo, Identi.ca, and more to work').'</div>
			</div>';
		}
	}
}
...
function networkpub_conf() {
	...
	$html .= 				   '<table style="width:100%;">
						...
						<div style="font-size:13px;margin:20px 0px 0px 0px;">
                            <div class="networkpublisher_header">
